...
defmodule BeamFsm.SchedulerTest do
  ...
  test "стартует с начальным значением" do
    pid = start_supervised!({BeamFsm.Scheduler, %{counter: 0, fact: 0, predicate: true, task: nil}})
  ...
  test "update_status" do
    pid = start_supervised!({BeamFsm.Scheduler, %{counter: 5, fact: 0, predicate: true, task: nil}})
    BeamFsm.Scheduler.update_status(pid, 7) 
    assert BeamFsm.Scheduler.get_status(pid) == 12
  end

    test "submit_task" do
    pid = start_supervised!({BeamFsm.Scheduler, %{counter: 5, fact: 0, predicate: true, task: fn x -> x + 1 end}})
